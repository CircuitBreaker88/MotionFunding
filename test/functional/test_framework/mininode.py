...
"""Motion P2P network half-a-node.
...
from test_framework.motionconfig import INITIAL_HASH_UTXO_ROOT, INITIAL_HASH_STATE_ROOT
...
class CTransaction(object):
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in motiond
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class NodeConnCB(object):
    """Callback and helper functions for P2P connection to a motiond node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Motion Node: %s:%d' % (self.dstaddr, self.dstport))
