...
//
// Bootup the masternode, look for a 16120 XMNF input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode() {
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Height()) * COIN) {  //exactly DARKSEND_COLLATERAL MOTION
...
vector <COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress) {
    ...
    // Filter
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == DARKSEND_COLLATERAL) { //exactly 161.200 XMNF
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
