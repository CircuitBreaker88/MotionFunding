...
#if defined(NDEBUG)
#error "MOTION cannot be compiled without assertions."
#endif
...
std::unique_ptr<MotionState> globalState = nullptr;
...
std::string SCVersion ("/Motion :5.2.5/");
...
unsigned int dgpMaxBlockSize = 2000000; // motion
...
const string strMessageMagic = "Motion Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            MotionDGP motionDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = motionDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = motionDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            MotionTxConverter converter(tx, NULL);
            ExtractMotionTX resultConverter;
            if(!converter.extractionMotionTransactions(resultConverter)){
            ...
            std::vector<MotionTransaction> motionTransactions = resultConverter.first;
            std::vector<EthTransactionParams> motionETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(MotionTransaction motionTransaction : motionTransactions){
                sumGas += motionTransaction.gas() * motionTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, motionTransaction.gasPrice());
                } else {
                    txMinGasPrice = motionTransaction.gasPrice();
                }
                VersionVM v = motionTransaction.getVersion();
                if(v.format!=0)
                ...
                if(motionTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(motionTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(motionTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += motionTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)motionTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(motionETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > motionTransactions.size())
...
#if 0
    MotionDGP motionDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setMotionSchedule(motionDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//motionDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//motionDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//motionDGP.getBlockSize(pindex->nHeight + 1);
        ...
        if (fAddressIndex) { // OUTPUTS
            for (size_t k = 0; k < tx.vout.size(); k++) {
                ...
                if (ExtractDestination(out.scriptPubKey, dest, &txType)) {
                    ...
                    // outputs like OP_RETURN custom data, some MOTION v3 vouts had 1 satoshi in nValue
...
///////////////////////////////////////////////////////////////////////////////////////// motion
        ...
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                MotionTxConverter convert(tx, &view, &block.vtx);

                ExtractMotionTX resultConvertMotionTX;
                if(!convert.extractionMotionTransactions(resultConvertMotionTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertMotionTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertMotionTX.first, blockGasLimit);
                ...
                for(MotionTransaction& ltx : resultConvertMotionTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertMotionTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertMotionTX.first[k].from(), resultConvertMotionTX.first[k].to(),
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 XMNF
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nHeight >= nMotionProtocolSwitchHeight)
    {
 ...
 * As an example, Motion Core uses /Motion :X.X.X/ version string. So, this function will
 * return 5 for /Motion :5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Motion Core wallet version as a string
 * @return Major Motion Core major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    MotionTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<MotionTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path motionDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(motionDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(motionDir.string(), std::ios::in | std::ios::out);
...
bool MotionTxConverter::extractionMotionTransactions(ExtractMotionTX& motiontx){
    std::vector<MotionTransaction> resultTX;
    ...
    motiontx = std::make_pair(resultTX, resultETP);
    return true;
}

bool MotionTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool MotionTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
MotionTransaction MotionTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    MotionTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = MotionTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = MotionTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
