...
bool MotionDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule MotionDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t MotionDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t MotionDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t MotionDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t MotionDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool MotionDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void MotionDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void MotionDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void MotionDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void MotionDGP::createParamsInstance(){
...
dev::Address MotionDGP::getAddressForBlock(unsigned int blockHeight){
...
void MotionDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MotionDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MotionDGP::parseStorageOneUint64(uint64_t& value){
...
void MotionDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule MotionDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
