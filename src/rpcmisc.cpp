...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized motion balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in motion/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in motion/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    MotionDGP motionDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)motionDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)motionDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)motionDGP.getBlockGasLimit(chainActive.Height())));
...
#ifdef ENABLE_WALLET
        // Case 1: XMNF address and we have full public key:
