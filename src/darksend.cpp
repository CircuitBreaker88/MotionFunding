    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.002Motion per transaction on average.
            */
            if (r <= 20) {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready) {
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeMotionAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if (!sessionFoundMasternode) {
        ...
        double fMotionSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Motion CENT - sessionTotalValue %d\n", fMotionSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom) {
    ...
    //
    // bit 0 - 100Motion+1 ( bit on if present )
    // bit 1 - 10Motion+1
    // bit 2 - 1Motion+1
    // bit 3 - .1Motion+1
    // bit 3 - non-denom
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0) {
            if (nLiquidityProvider != 0) {
                ...
                if (nRand == 50 + nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8) {
                    ...
                    nAnonymizeMotionAmount = (rand() % nLeftToAnon) + 3;
                } else {
