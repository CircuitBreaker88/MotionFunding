...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setMotionSchedule(motionDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = motionDGP.getBlockSize(nHeight);
    minGasPrice = motionDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = motionDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    MotionTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractMotionTX resultConverter;
    if(!convert.extractionMotionTransactions(resultConverter)){
    ...
    for(MotionTransaction motionTransaction : motionTransactions){
        txGas += motionTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + motionTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(motionTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, motionTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MOTIONMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("MOTIONMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("MOTIONMiner : ProcessNewBlock, block not accepted");
    }
