...
void DownloadManager::startNextDownload()
{
    ...
    if (currentRequest.checkSize) { // get only file size
        ...
        if (!output.open(QIODevice::WriteOnly)) {
            ...
            DownloadManager::DownloadProgress progress("Motion Wallet", "Could not create file for download!");
...
void DownloadManager::onDownloadProgress(qint64 bytesReceived, qint64 bytesTotal)
{
    ...
    DownloadManager::DownloadProgress progress("Motion Wallet", bytesReceived, bytesTotal);
...
void DownloadManager::onDownloadFinished()
{
    if (!currentRequest.checkSize) {
        ...
        if (currentDownload->error()) {
            output.remove();
            DownloadManager::DownloadProgress progress("Motion Wallet", currentDownload->errorString());
            emit downloadFinished(progress);
        } else {
            ...
            if (isHttpRedirect()) {
                ...
                DownloadManager::DownloadProgress progress("Motion Wallet", 0, 0);
                emit downloadFinished(progress);
                ++downloadedCount;
            }
        }
        ...
        if (currentDownload->error() && currentDownload->error() != QNetworkReply::OperationCanceledError) {
            DownloadManager::DownloadProgress progress("Motion Wallet", -1, -1);
            emit downloadFinished(progress);
        } else {
            // let's check if it was actually a redirect
            if (isHttpRedirect()) {
                ...
                DownloadManager::DownloadProgress progress("Motion Wallet", -1, totalSize);
                emit downloadFinished(progress);
            }
        }
    }
