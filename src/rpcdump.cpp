...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fRescan) {
        ...
        if (pwalletMain->IsAbortingRescan()) {
            msg =  _("Rescan aborted by user. Please restart your Motion Core wallet with '-rescan' option. Otherwise, transaction data");
        ...
        {
            msg =  _("'importprivkey' with 'Rescan' option finished successfully. Please restart your Motion Core wallet. Otherwise, transaction data");
            msg += _(" or address book entries might be missing or incorrect");
        }
    }
    else
    {
        msg =  _("'importprivkey' finished successfully. Please restart Motion Core wallet with '-rescan' option. Otherwise, transaction data");
        msg += _(" or address book entries might be missing or incorrect");
    }
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid XMNF address or script");
    }
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by MOTION %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"motionaddress\"\n"
            "\nEncrypts a private key corresponding to 'motionaddress'.\n"
            "\nArguments:\n"
            "1. \"motionaddress\"   (string, required) The motion address for the private key (you must hold the key already)\n"
