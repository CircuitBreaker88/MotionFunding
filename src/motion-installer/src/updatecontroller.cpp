...
#include "motionupdater/motionupdater_p.h"

std::atomic<bool> running(false);
std::atomic<bool> wasCanceled(false);
QPointer<QWidget> win;
QtMotionUpdater::UpdateController::DisplayLevel gDisplayLevel;
QtMotionUpdater::ProgressDialog *gUpdatesProgress;
...
MotionUpdater *UpdateController::updater() const
{
	...
	if(!d->mainUpdater->checkForUpdates()) {
		...
		if(gDisplayLevel >= ExtendedInfoLevel) {
			...
			if(gDisplayLevel >= ProgressLevel) {
				...
				d->checkUpdatesProgress->open(d->mainUpdater, &QtMotionUpdater::MotionUpdater::abortUpdateCheck);
			}
		}
		return true;
	}
...
void UpdateController::checkUpdatesDone(bool hasUpdates, bool hasError)
{
	...
	if(wasCanceled) {
		...
		if(hasUpdates) {
			...
			if(hasError) {
				qCWarning(logMotionUpdater) << "maintenancetool process finished with exit code"
...
{
	return QIcon::fromTheme(QStringLiteral("system-software-update"), QIcon(QStringLiteral(":/res/icons/motionupdate.ico")));
}
...
UpdateControllerPrivate::UpdateControllerPrivate(UpdateController *q_ptr, const QString &version, QWidget *window) :
	q(q_ptr),
	window(window),
	mainUpdater(version.isEmpty() ? new MotionUpdater(q_ptr) : new MotionUpdater(version, q_ptr)),
	runAdmin(true),
	adminUserEdit(true),
	...
	scheduler(new SimpleScheduler(q_ptr))
{
	...
	QObject::connect(mainUpdater, &MotionUpdater::checkUpdatesDone,
...
{
	if(running)
		qCWarning(logMotionUpdater) << "UpdaterController destroyed while still running! This can crash your application!";
