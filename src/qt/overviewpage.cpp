...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
            QAbstractItemDelegate(), unit(BitcoinUnits::MOTION),
                                              ...
                                              filter(0)
{
    ...
    ui->setupUi(this);
    //set Logo
    ui->labelLogo->setPixmap(QPixmap(":/images/motion_logo_horizontal")
...
#else
       {
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ...
                ui->toggleDarksend->setText(tr("Start Motionsend"));
            } else {
                ui->darksendEnabled->setText(tr("Enabled"));
                ui->toggleDarksend->setText(tr("Stop Motionsend"));
            }
...
void OverviewPage::updateDarkSendProgress()
{
    ...
    QString strAnonymizeMotionAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMotionAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (!fEnableDarksend) {
        ...
        strAnonymizeMotionAmount = strAnonymizeMotionAmount.remove(strAnonymizeMotionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeMotionAmount * COIN) nMaxToAnonymize = nAnonymizeMotionAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeMotionAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeMotionAmount));
        strAnonymizeMotionAmount = strAnonymizeMotionAmount.remove(strAnonymizeMotionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::darkSendStatus()
{
    ...
    if (strStatus != ui->darkSendStatus->text() && !strStatus.isEmpty()) {
        LogPrintf("Last Motionsend message: %s\n", strStatus.toStdString().c_str());
