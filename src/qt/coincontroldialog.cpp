    ...
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (coinControl->useDarksend && rounds < nDarksendRounds)
            QMessageBox::warning(this, windowTitle(),
                                 tr("Non-anonymized input selected. <b>Motionsend will be disabled.</b><br><br>If you still want to use Motionsend, please deselect all non-nonymized inputs first and then check Motionsend checkbox again."),
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::MOTION;
    ...
    for (PAIRTYPE(QString, vector<COutput>) coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show XMNF address. In tree mode, address is not shown again for direct wallet address outputs
