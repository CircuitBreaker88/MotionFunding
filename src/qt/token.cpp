...
bool ToHash160(const std::string& strMotionAddress, std::string& strHash160)
{   
    CTxDestination motionAddress = DecodeDestination(strMotionAddress);
    if(IsValidDestination(motionAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&motionAddress);
...
bool ToMotionAddress(const std::string& strHash160, std::string& strMotionAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strMotionAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToMotionAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToMotionAddress(tokenEvent.receiver, tokenEvent.receiver);
