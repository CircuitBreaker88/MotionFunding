...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasMotionAddress = IsValidDestination(addr);
        if (hasMotionAddress) {
            ui->label->setText("Address (Motion)");
        ...
        isAddressValid = hasHexAddress || hasMotionAddress;
    }
