...
//    SetObjectStyleSheet(ui->pushButtonClearAll, StyleSheetNames::ButtonBlack);
    ...
    ui->labelAmount->setToolTip(tr("The amount in Motion to send. Default = 0."));
...
void SendToContract::on_numBlocksChanged(int newHeight)
{
    if(m_clientModel)
    {
        ...
        if (lastUpdatedHeight < newHeight) {
            ...
            ui->labelGasPrice->setToolTip(tr("Gas price: MOTION price per gas unit. Default = %1, Min = %2.").arg(
