...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool sign)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        ...
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered motion address / amount:
...

    {
        ...
        if (recipients[0].useInstanTX && total > GetSporkValue(SPORK_2_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("InstanTX doesn't support sending values that high yet. Transactions are currently limited to %1 XMNF.").arg(GetSporkValue(SPORK_2_MAX_VALUE)),
...
bool WalletModel::isUnspentAddress(const std::string &motionAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    for (const COutput& out : vecOutputs)
    {
        ...
        if(fValidAddress && EncodeDestination(address) == motionAddress && out.tx->vout[out.i].nValue)
        {
            return true;
        }
    }
