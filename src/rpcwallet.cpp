...
// Copyright (c) 2015-2017 The Motion Developers
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns a new XMNF address for receiving payments.\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current XMNF address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new XMNF address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid XMNF address");
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"motionaddress\" amount ( \"comment\" \"comment-to\" subtractfee )\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddressix \"motionaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"motionaddress\"  (string, required) The motion address to send to.\n"
...
UniValue listaddressbalances(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"address\": amount, (string) Motion address and the amount in " + CURRENCY_UNIT + "\n"
            "  ,...\n"
            "}\n"
            "\nExamples:\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"motionaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given motionaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tomotionaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a motion address.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The motion address is the key, the numeric amount in motion is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const string& name_ : keys) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid XMNF address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a XMNF address or hex-encoded public key.\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending XMNFs\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending XMNFs\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
    ...
    StartShutdown();
    return "wallet encrypted; motion server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending XMNFs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in XMNF/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "Wallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same XMNF address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <XMNF Address> <percent>\n"
...
UniValue callcontract(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 3){
        CTxDestination motionSenderAddress = DecodeDestination(params[2].get_str());
        if(IsValidDestination(motionSenderAddress)) {
            CKeyID *keyid = boost::get<CKeyID>(&motionSenderAddress);
...
UniValue createcontract(const UniValue& params, bool fHelp){
    ...
    uint64_t blockGasLimit = motionDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(motionDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (fHelp || params.size() < 1 || params.size() > 6)
        throw runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice MOTION price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
    ...
    if (params.size() > 3){
        ...
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Motion address to send from");
        else
            fHasSender=true;
    }
...
UniValue sendtocontract(const UniValue& params, bool fHelp){
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Motion price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
