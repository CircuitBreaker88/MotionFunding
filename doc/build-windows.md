...
Below are some notes on how to build Motion Core for Windows.

The options known to work for building Motion Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Motion Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Motion Core applications.
...
Note that for WSL the Motion Core source path MUST be somewhere in the default mount file system, for
example /usr/src/motion, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\motion`, for example:

    make install DESTDIR=/mnt/c/workspace/motion
...
causes two of the motion executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the motion code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the motion source code).
