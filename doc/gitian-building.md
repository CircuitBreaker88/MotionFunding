...
*Setup instructions for a gitian build of MOTION using a Debian VM or physical system.*
...
- [Building MOTION](#building-motion)
...
Clone the git repositories for motion and gitian and then checkout the motion version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the motion repository
...
To build MOTION (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the motion repository.
...
```bash
    ...
    From https://github.com/216k155/motion
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/motion-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/motion-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/motion-build.assert
```
...
[motion/gitian.sigs](https://github.com/216k155/gitian.sigs/) repository, or if that's not possible to create a pull
request.
